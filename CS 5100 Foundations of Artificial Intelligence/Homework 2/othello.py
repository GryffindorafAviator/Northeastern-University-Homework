...
def minimax_value(board, white_turn, search_depth):
    if search_depth == 0:
        #print("0 board")
        #print_board(board)
        white_count = np.count_nonzero(board == WHITE)
    ...
    if check_game_over(board) != NOBODY:
        if check_game_over(board) == WHITE:
            #print("winner:", check_game_over(board))
            return WIN_VAL
    ...
    if white_turn:
        ...
        if legal_moves:
            best_val = float("-inf")
            best_move = None
            #print("for a white step")
            for m in legal_moves:
                new_board = play_move(board, m, True)
                print_board(new_board)
                #print("search count for depth: ", search_depth, "count: ", m)
                #print("search for black")
                move_val = minimax_value(new_board, False, search_depth - 1)
                #print("search for black over")
            ...
            return minimax_value(board, False, search_depth)
        ...
        if legal_moves:
            best_val = float("inf")
            best_move = None
            #print("for a black step")
            for m in legal_moves:
                new_board = play_move(board, m, False)
                print_board(new_board)
                #print("search count for depth: ", search_depth, "count: ", m)
                #print("search for white")
                move_val = minimax_value(new_board, True, search_depth - 1)
                #print("search for white over")
                if move_val < best_val:
                    best_move = m
                    best_val = move_val
            # board = play_move(board, best_move, False)
            return best_val
        else:
            return minimax_value(board, True, search_depth)


# Alpha-Beta version
def minimax_value_ab(board, white_turn, search_depth, alpha, beta):
    #print("continue..")
    #print("...")
    if search_depth == 0:
        #print("0 board")
    ...
    if white_turn:
        ...
        if legal_moves:
            ...
            for m in legal_moves:
                ...
                #print("search count for depth: ", search_depth, "count: ", m)
                #print("search for black")
                move_val = minimax_value_ab(new_board, False, search_depth - 1, float("-inf"), float("inf"))
                #print("search for black over")
                if move_val > alpha:
                    best_move = m
                    alpha = move_val
            ...
            return minimax_value_ab(board, False, search_depth, float("-inf"), float("inf"))
        ...
        if legal_moves:
            ...
            for m in legal_moves:
                ...
                #print("search count for depth: ", search_depth, "count: ", m)
                #print("search for white")
                move_val = minimax_value_ab(new_board, True, search_depth - 1, float("-inf"), float("inf"))
                #print("search for white over")
                if move_val < beta:
                    best_move = m
                    beta = move_val
            ...
            return minimax_value_ab(board, True, search_depth, float("-inf"), float("inf"))
...
if firstline == "play":
    ...
    board = read_board()
    print(minimax_value_ab(board, WHITE_TO_PLAY, search_depth, float("-inf"), float("inf")))
